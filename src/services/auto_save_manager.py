...
class AutoSaveManager:
        ...
        try:
            ...
            if categoria in ['analise_completa', 'pesquisa_web'] and len(str(dados)) > 1000:
                ...
                with open(json_filepath, "w", encoding="utf-8") as f:
                    # Serializa dados para JSON com proteção contra referências circulares
                    try:
                        # Remove referências circulares antes da serialização
                        dados_limpos = self._clean_circular_references(dados)
                        json_content = json.dumps(dados_limpos, indent=2, ensure_ascii=False, default=str)
                    except (TypeError, ValueError) as e:
                        logger.error(f"❌ Erro na serialização JSON: {str(e)}")
                        json_content = json.dumps({
                            'erro_serializacao': str(e),
                            'dados_originais': str(dados)[:1000],
    ...
    def _garantir_pasta_existe(self, caminho_pasta: str):
        """Garante que a pasta existe"""
        try:
            if not os.path.exists(caminho_pasta):
                os.makedirs(caminho_pasta, exist_ok=True)
                logger.debug(f"📁 Pasta criada: {caminho_pasta}")
        except Exception as e:
            logger.error(f"❌ Erro ao criar pasta {caminho_pasta}: {str(e)}")

    def _clean_circular_references(self, obj, seen=None, depth=0):
        ...
        if depth > 10:
            return f"<Profundidade máxima atingida: {type(obj).__name__}>"

        obj_id = id(obj)
        if obj_id in seen:
            return f"<Referência circular detectada para {type(obj).__name__}>"
        ...
        try:
            if isinstance(obj, dict):
                ...
                for k, v in obj.items():
                    try:
                        if isinstance(k, str) and len(k) < 1000:  # Limita tamanho da chave
                            cleaned[k] = self._clean_circular_references(v, seen.copy(), depth + 1)
                    except Exception as e:
                        cleaned[k] = f"<Erro ao processar: {str(e)[:100]}>"
                return cleaned
            elif isinstance(obj, (list, tuple)):
                cleaned = []
                for i, item in enumerate(obj[:100]):  # Limita a 100 itens
                    try:
                        cleaned.append(self._clean_circular_references(item, seen.copy(), depth + 1))
                    except Exception as e:
                        cleaned.append(f"<Erro no item {i}: {str(e)[:100]}>")
                return cleaned
            elif hasattr(obj, '__dict__'):
                return f"<Objeto {type(obj).__name__}>"
            elif isinstance(obj, (str, int, float, bool, type(None))):
                return obj
            else:
                return str(obj)[:1000]  # Limita strings muito longas
        except Exception as e:
            return f"<Erro na limpeza: {str(e)[:100]}>"
        finally:
            seen.discard(obj_id)
...
def safe_serialize_for_save(obj: Any, visited: set = None, depth: int = 0) -> Any:
    """Serialização ultra-segura para salvamento"""
    if visited is None:
        visited = set()

    if depth > 10:
        return {"__max_depth__": "Limite de profundidade atingido"}

    obj_id = id(obj)
    if obj_id in visited:
        return {"__circular_ref__": f"{type(obj).__name__}"}
    ...
    try:
        ...
        if isinstance(obj, dict):
            ...
            for key, value in obj.items():
                ...
                try:
                    result[safe_key] = safe_serialize_for_save(value, visited.copy(), depth + 1)
                except:
                    result[safe_key] = "<Serialization error>"
            return result

        if isinstance(obj, (list, tuple)):
            return [safe_serialize_for_save(item, visited.copy(), depth + 1) for item in obj[:20]]

        return str(obj)[:500]

    except:
        return {"__error__": f"Failed to serialize {type(obj).__name__}"}
    finally:
        visited.discard(obj_id)

# Função de conveniência
def salvar_etapa(nome_etapa: str, dados: Any, status: str = "sucesso", categoria: str = "geral") -> str:
    """Função de conveniência para salvamento rápido com serialização segura"""
    try:
        # Aplica serialização segura nos dados
        dados_seguros = safe_serialize_for_save(dados)
        return auto_save_manager.salvar_etapa(nome_etapa, dados_seguros, status, categoria=categoria)
    except Exception as e:
        logger.error(f"Erro ao salvar etapa {nome_etapa} com serialização segura: {e}")
        # Tenta salvar versão simplificada em caso de falha na serialização
        dados_simples = {
            "etapa": nome_etapa,
            "status": "erro_serializacao",
            "erro": str(e)[:200],
            "timestamp": datetime.now().isoformat()
        }
        return auto_save_manager.salvar_etapa(f"ERRO_{nome_etapa}", dados_simples, status="erro", categoria="erros")
