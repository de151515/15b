...
# Tentativa de importar firecrwal_social_client e social_media_extractor
firecrwal_social_client = None
social_media_extractor = None
...
try:
    ...
    logger.info("✅ Módulos de social media importados com sucesso")
except ImportError as e:
    logger.warning(f"⚠️ Falha ao importar módulos de social media: {e}. Funcionalidades de busca massiva podem estar indisponíveis.")
except Exception as e:
    logger.error(f"❌ Erro inesperado ao importar social media: {e}")
...
class MCPSupadataManager:
    ...
    def search_youtube(self, query: str, max_results: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"⚠️ YouTube API 401: Credenciais inválidas ou expiradas")
                return self._create_simulated_youtube_data(query, max_results)
            else:
    ...
    def search_massive_social_media(self, query: str, use_firecrwal: bool = True) -> Dict[str, Any]:
        """Executa busca massiva em redes sociais com Firecrwal"""

        if use_firecrwal and firecrwal_social_client and firecrwal_social_client.enabled:
            logger.info("🔥 Executando busca massiva com Firecrwal")

            firecrwal_results = firecrwal_social_client.search_social_media_massively(
                query=query,
                platforms=['youtube', 'twitter', 'instagram', 'linkedin', 'tiktok', 'facebook']
            )

            # Adiciona análise de sentimento aos resultados do Firecrwal
            all_posts = []

            for platform, data in firecrwal_results.get('platform_results', {}).items():
                if data.get('results'):
                    all_posts.extend(data['results'])

            sentiment_analysis = self.analyze_sentiment(all_posts)

            return {
                'firecrwal_results': firecrwal_results,
                'sentiment_analysis': sentiment_analysis,
                'total_insights': firecrwal_results.get('total_insights', 0),
                'extraction_method': 'firecrwal_massive',
                'generated_at': datetime.now().isoformat()
            }
        else:
            logger.info("🔄 Firecrwal não disponível ou desabilitado, usando busca padrão")
            # Chama a função original de busca em todas as plataformas, se disponível
            if social_media_extractor:
                 return social_media_extractor.search_all_platforms(query, max_results_per_platform=15)
            else:
                 # Fallback para a busca interna se social_media_extractor não foi importado
                 return self.search_all_platforms_fallback(query, max_results_per_platform=15)
    ...
    # Método de fallback para search_all_platforms caso social_media_extractor não seja importado
    def search_all_platforms_fallback(self, query: str, max_results_per_platform: int = 10) -> Dict[str, Any]:
        """Busca UNIFICADA em todas as plataformas (Fallback)"""
        logger.warning("Usando fallback para search_all_platforms devido a falha na importação.")
        return self.search_all_platforms(query, max_results_per_platform)
