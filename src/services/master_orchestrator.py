...
class MasterOrchestrator:
    ...
    def _generate_visual_proofs(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                # Chama método com parâmetros corretos
                visual_results['experiences'] = visual_proofs_director.create_transformative_experience(
                    data.get('avatar_detalhado', {}),
                    data.get('mental_drivers', {}),
                    data.get('web_research', {}),
                    data.get('social_analysis', {})
                )
            except Exception as e:
                logger.error(f"❌ Erro no visual_proofs_director: {str(e)}")
                visual_results['experiences'] = {'error': str(e), 'fallback': True}
