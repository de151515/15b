...
from services.alibaba_websailor import alibaba_websailor  # Importando a instância global do WebSailor
...
class SuperOrchestrator:
    ...
    def __init__(self):
        ...
        self.orchestrators = {
            ...
            'websailor': alibaba_websailor  # Substituído production_search pelo websailor
        }
    ...
    def _execute_web_search(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Executa pesquisa web sincronizada usando Alibaba WebSailor Agent"""

        try:
            query = data.get('query') or f"mercado {data.get('segmento', '')} {data.get('produto', '')} Brasil 2024"
            session_id = data.get('session_id', 'default')

            logger.info(f"🔍 Iniciando navegação profunda com Alibaba WebSailor: {query}")

            # Usa Alibaba WebSailor Agent para pesquisa profunda
            search_results = alibaba_websailor.navigate_and_research_deep(
                query, 
                data,
                max_pages=25,  # Ajustado para usar o máximo do WebSailor
                depth_levels=3,
                session_id=session_id
            )

            # Converte os resultados do AlibabaWebSailorAgent para a estrutura esperada
            if isinstance(search_results, dict) and 'conteudo_consolidado' in search_results:
                fontes_detalhadas = search_results['conteudo_consolidado'].get('fontes_detalhadas', [])
                results_list = [{
                    'title': fonte['title'],
                    'url': fonte['url'],
                    'snippet': fonte.get('snippet', '') or ' '.join(search_results['conteudo_consolidado']['insights_principais'][:2])[:200],
                    'source': fonte['search_engine'],
                    'quality_score': fonte.get('quality_score', 0)
                ...
                search_results = {
                    ...
                    'original_alibaba_results': search_results,
                    'quality_metrics': {
                        'avg_quality_score': search_results['navegacao_profunda'].get('qualidade_media', 0),
                        'preferred_sources': search_results['navegacao_profunda'].get('fontes_preferenciais', 0)
                    }
                }
            else:
                # Caso de falha ou formato inesperado
                search_results = {
                    'results': [],
                    'query': query,
                    'total': 0,
            ...
            final_results = search_results.get('results', [])

            # Se não tiver resultados suficientes, tenta usar enhanced_search_coordinator
            if not final_results or len(final_results) < 5:
                logger.info("🔄 Poucos resultados - usando enhanced search coordinator como fallback")
                try:
                    ...
                    if enhanced_results and isinstance(enhanced_results, dict):
                        # Ajusta para a estrutura esperada
                        ...
                        search_results = {
                            ...
                            'total': len(enhanced_results),
                            'quality_metrics': {
                                'avg_quality_score': 70,  # Valor médio para fallback
                                'preferred_sources': 0
                            }
                        }
                except Exception as e:
            ...
            return {
                ...
                'websailor_stats': {
                    'total_pages_analyzed': search_results.get('original_alibaba_results', {}).get('navegacao_profunda', {}).get('total_paginas_analisadas', 0),
                    'insights_count': search_results.get('original_alibaba_results', {}).get('conteudo_consolidado', {}).get('insights_principais', []),
                    'quality_score': search_results.get('quality_metrics', {}).get('avg_quality_score', 0)
                }
            }

        except Exception as e:
            logger.error(f"❌ Erro na pesquisa web com Alibaba WebSailor: {e}")
            return {
                'error': str(e), 
                'fallback_used': True,
                'search_results': {
...
- ✅ Pesquisa Web Massiva (Alibaba WebSailor)
