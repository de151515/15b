...
class UltraDetailedAnalysisEngine:
    ...
    def _execute_avatar_analysis(self, data: Dict[str, Any], research_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            avatar_analysis_clean = self._clean_json_response(response)
            avatar_obj = json.loads(avatar_analysis_clean)

            if not self._validate_avatar_structure(avatar_obj):
                logger.warning("⚠️ Estrutura do avatar inválida, usando fallback")
                return self._create_fallback_avatar()

            logger.info("✅ Avatar ultra-detalhado gerado com sucesso")
            return avatar_obj
        except json.JSONDecodeError as e:
            logger.error(f"❌ Avatar inválido retornado pela IA: {str(e)}")
            logger.info("🔄 Usando avatar de fallback")
            return self._create_fallback_avatar()
        except Exception as e:
            logger.error(f"❌ Erro inesperado no avatar: {str(e)}")
            return self._create_fallback_avatar()
    ...
    def _validate_avatar_structure(self, avatar_data: Any) -> bool:
        """Valida estrutura mínima do avatar"""
        if not isinstance(avatar_data, dict):
            return False

        required_fields = ['nome', 'idade', 'demograficos', 'psicograficos']
        return all(field in avatar_data for field in required_fields)

    def _clean_json_response(self, response: str) -> str:
        """Limpa resposta da IA para melhorar parsing JSON"""
        try:
            # Remove prefixos e sufixos comuns da IA
    ...
    def _create_fallback_avatar(self) -> Dict[str, Any]:
        """Cria avatar de fallback quando IA falha"""
        return {
            'nome': 'Avatar Padrão',
            'idade': '30-45 anos',
            'demograficos': {
                'genero': 'Diversificado',
                'renda': 'Classe média',
                'educacao': 'Superior completo',
                'localizacao': 'Centros urbanos'
            },
            'psicograficos': {
                'personalidade': 'Pragmático e orientado a resultados',
                'valores': 'Eficiência e qualidade',
                'estilo_vida': 'Ativo e conectado',
                'motivacoes': 'Sucesso profissional e pessoal'
            },
            'comportamentos': {
                'online': 'Pesquisa antes de comprar',
                'compra': 'Baseado em benefícios claros',
                'comunicacao': 'Prefere informações diretas'
            },
            'fallback': True,
