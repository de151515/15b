...
def analyze():
    ...
    try:
        ...
        # Aplica serialização segura na resposta final
        try:
            from services.auto_save_manager import safe_serialize_for_save
            
            # Serializa dados de forma segura
            safe_resultado = safe_serialize_for_save(resultado)
            ...
            return jsonify({
                ...
                'processing_time': safe_resultado.get('metadata', {}).get('processing_time_formatted', 'N/A'),
                'data': safe_resultado,
                'engine': 'ARQV30 Enhanced v3.0 - ULTRA SECURE',
                'clean_report_available': 'relatorio_final_limpo' in safe_resultado,
                'serialization': 'safe'
            })
            
        except Exception as serialize_error:
            logger.error(f"Erro na serialização final: {serialize_error}")
            
            # Fallback para resposta mínima segura
            return jsonify({
                'success': True,
                'session_id': session_id,
                'message': 'Análise concluída com dados simplificados',
                'data': {
                    'status': 'completed_with_serialization_fallback',
                    'session_id': session_id,
                    'timestamp': datetime.now().isoformat(),
                    'note': 'Dados completos salvos no sistema, resposta simplificada devido a complexidade estrutural'
                },
                'engine': 'ARQV30 Enhanced v3.0 - SAFE MODE',
                'serialization': 'fallback'
            })

    except Exception as e:
            logger.error(f"❌ Erro na análise: {e}")
            salvar_erro("erro_analise", str(e))
